...
function GenerationResult({
  ...
  result: TimetableGenerationViewModel;
}) {
  const t = useTranslations("academics.timetable.generate");
  ...
  return (
    <div className="space-y-4">
      <div
        className={`flex gap-2 rounded-lg border p-3 text-sm ${isComplete ? "border-green-200 bg-green-50 text-green-900" : "border-amber-200 bg-amber-50 text-amber-900"}`}
        role="status"
      >
      ...
      <dl className="grid grid-cols-3 gap-3 rounded-lg bg-gray-50 p-4 text-sm">
        <ScopeDetail
          label={t("result.createdCount")}
          value={String(result.createdCount)}
        />
        <ScopeDetail
          label={t("result.existingCount")}
          value={String(result.existingCount)}
        />
        <ScopeDetail
          label={t("result.remainingDemandCount")}
          value={String(result.remainingDemandCount)}
        />
      </dl>
      {result.groups.map((group) => (
        <section
          key={group.classroomId ?? "unassigned"}
        ...
        >
          ...
          <ul className="mt-2 space-y-2 text-sm text-gray-700">
            {group.items.map((unresolved) => (
              <li
                key={`${unresolved.code}:${unresolved.subjectId ?? "unassigned"}`}
