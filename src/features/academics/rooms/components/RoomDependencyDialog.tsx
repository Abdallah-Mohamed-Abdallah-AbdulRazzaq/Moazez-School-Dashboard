...
export default function RoomDependencyDialog({ error, onClose }: { error: RoomSchedulingUiError | null; onClose: () => void }) {
  return (
    <Modal isOpen={Boolean(error)} onClose={onClose} title={error?.message} size="sm" footer={<Button onClick={onClose}>{"Close"}</Button>}>
      <dl className="space-y-2 text-sm text-gray-700">
        {error?.operation && <div><dt className="font-medium">Operation</dt><dd>{error.operation}</dd></div>}
        {Object.entries(error?.dependencyCounts ?? {}).map(([label, count]) => <div key={label}><dt className="font-medium">{label}</dt><dd>{count}</dd></div>)}
        {error?.traceId && <div><dt className="font-medium">Trace ID</dt><dd>{error.traceId}</dd></div>}
      </dl>
    </Modal>
  );
}
